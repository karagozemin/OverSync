...
import { compareStatus, isStaleTransition } from "../src/state-machine/order-machine.js";
...
describe("monotonic order status helpers", () => {
  it("identifies delayed lifecycle transitions", () => {
    expect(isStaleTransition("dst_locked", "src_locked")).toBe(true);
    expect(isStaleTransition("src_locked", "dst_locked")).toBe(false);
    expect(compareStatus("announced", "secret_revealed")).toBe(-1);
    expect(compareStatus("completed", "refunded")).toBe(0);
  });
});
